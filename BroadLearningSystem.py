...
#def plot_confusion_matrix(OutputOfTest, test_y, classes=range(10), cmap=plt.cm.Blues):
    # 获取预测标签
    y_pred = np.argmax(OutputOfTest, axis=1)
...
def pinv(A, reg):
    return np.mat(reg*np.eye(A.shape[1])+A.T.dot(A)).I.dot(A.T)#计算矩阵带正则化的伪逆
...
def BLS(train_x, train_y, test_x, test_y, s, c, N1, N2, N3):
    ...
    FeatureOfInputDataWithBias = np.hstack([train_x, 0.1 * np.ones((train_x.shape[0],1))])#在输入矩阵中加入一个偏置项0.1
    OutputOfFeatureMappingLayer = np.zeros([train_x.shape[0], N2*N1])#特征映射层的输出矩阵
    Beta1OfEachWindow = []
...
#    print('Enhance nodes: max:',np.max(tempOfOutputOfEnhanceLayer),'min:',np.min(tempOfOutputOfEnhanceLayer))
    ...
    #plot_confusion_matrix(OutputOfTest, test_y) 生成混淆矩阵
